        ...
        switch (mode) {
            case INTERVAL -> {
                if (value instanceof Integer v) {
                    ...
                    QuickbackupmultiReforged.logger.error("Failed to build schedule trigger", buildException);
                }
            }
            case CRONTAB -> {
                if (value instanceof String v) {
                    if (!cronIsValid(v)) {
                        QuickbackupmultiReforged.logger.error("Failed to build schedule trigger, CronExpression {} is invalid", v);
                        return null;
                    }
                    return TriggerBuilder.newTrigger()
                        .withIdentity(name)
